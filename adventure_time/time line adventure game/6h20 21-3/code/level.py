...
class Level:
    def __init__(self,current_level,surface,create_overworld):
        ...
        # self.terrain_moving_sprites = self.create_tile_group(terrain_move_layout,'terrain_moving')
        # self.terrain_moving_sprites = self.setup_terrain_moving(terrain_move_layout)
    ...
    def setup_terrain_moving(self,layout):
        # terrain_move_sprite = pygame.sprite.Group()
        sprite_width = 3 * tile_size
        sprite_height = tile_size
        for row_index, row in enumerate(layout):
            for col_index,val in enumerate(row):
                if val != '-1' :#xem lại lỗi nằm ở đây
                    x = col_index * tile_size
                    y = row_index * tile_size
                    terrain_moving_tile_list = pygame.image.load('graphics/terrain/terrain_moving.png').convert_alpha()
                    # terrain_moving_tile_list = import_cut_graphics('graphics/terrain/terrain_moving.png')
                    # tile_surface = terrain_moving_tile_list[int(val)]
                    sprite = Terrain_Moving((sprite_width,sprite_height),x,y,terrain_moving_tile_list)
    ...
    def terrain_moving_collision_reverse(self):
        ...
        for terrain_move in self.terrain_moving_sprites:
            colliding_constraints = pygame.sprite.spritecollide(terrain_move, self.constraints_sprites, False)
            if colliding_constraints:
                for tmp in self.terrain_moving_sprites:
                    tmp.reverse()
    #####
        # colliding_sprites_0 = pygame.sprite.spritecollide(self.terrain_moving_sprites.sprites()[0], self.constraints_sprites, False)
        # colliding_sprites_2 = pygame.sprite.spritecollide(self.terrain_moving_sprites.sprites()[2], self.constraints_sprites, False)
        # colliding_sprites = colliding_sprites_0 + colliding_sprites_2

        # if colliding_sprites :
        #     print('0')
        #     for sprite in self.terrain_moving_sprites.sprites():
        #         sprite.reverse()

        # elif colliding_sprites_2 :
        #     print('2')
        #     for sprite in self.terrain_moving_sprites.sprites():
        #         sprite.reverse()
        #####
        # for  terrain_move in self.terrain_moving_sprites:
        # if colliding_sprites:
        #         self.terrain_moving_sprites.sprites()[0].reverse()
        #         self.terrain_moving_sprites.sprites()[1].reverse()
        #         self.terrain_moving_sprites.sprites()[2].reverse()
